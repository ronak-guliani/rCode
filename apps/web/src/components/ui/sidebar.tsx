  ...
  return (
    <SidebarInstanceContext value={instanceContextValue}>
      ...
      >
        ...
        <div
          className={cn(
            "fixed inset-y-0 z-10 hidden h-svh w-(--sidebar-width) transition-[left,right,width] duration-120 ease-linear md:flex",
            side === "left"
...
function SidebarGroupLabel({ className, render, ...props }: useRender.ComponentProps<"div">) {
  const defaultProps = {
    className: cn(
      "flex h-8 shrink-0 items-center rounded-lg px-2 font-medium text-sidebar-foreground text-xs outline-hidden ring-ring transition-[margin,opacity] duration-120 ease-linear focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0",
